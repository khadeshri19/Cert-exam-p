...
const verifyCertificate = async (verificationCode) => {
    const result = await db_1.default.query(`SELECT 
     ...
     WHERE vl.verification_code = $1`, [verificationCode]);
...
const getVerificationStatus = async (canvasSessionId, userId) => {
    const result = await db_1.default.query(`SELECT 
      cs.id as canvas_id,
     ...
     WHERE cs.id = $1 AND cs.user_id = $2`, [canvasSessionId, userId]);
    if (result.rows.length === 0) {
